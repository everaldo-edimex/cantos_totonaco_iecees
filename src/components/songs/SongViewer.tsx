...
export function SongViewer({ song, fontSize }: { song: Canto; fontSize: FontSize }) {
  ...
  return <div className={`song-viewer font-${fontSize}`}>{song.versos.map((verse, index) => verse.tipo === "coro" ? <ChorusBlock key={index} content={verse.contenido} /> : <VerseBlock key={index} order={verse.orden} content={verse.contenido} />)}</div>;
}
