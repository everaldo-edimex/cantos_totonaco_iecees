...
export default function PresentationPage() {
  ...
  return <div className="projector-view"><div className="presentation-progress"><span style={{ width: `${((index + 1) / total) * 100}%` }} /></div><main key={index} className={`${verse?.tipo ? "is-chorus" : ""} ${isTitle ? "is-title-slide" : ""}`}><span>{isTitle ? `Canto ${song.numero}` : verse?.tipo === "ultimo_coro" ? "Último coro" : verse?.tipo === "coro" ? "Coro" : verse?.orden === undefined ? "Verso" : `Verso ${verse.orden}`}</span>{isTitle ? <h1>{song.titulo}</h1> : <p>{verse?.contenido}</p>}</main><footer><span>{song.numero} · {song.titulo}</span><small>{index + 1} / {total}</small></footer>{showFullscreen && <button className="projector-fullscreen" onClick={enterFullscreen}><Maximize2 />Pantalla completa</button>}</div>;
}
